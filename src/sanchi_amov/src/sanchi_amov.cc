...
int main(int argc, char** argv)
{
  ...
  if(model == "100D2")
   {
	   uint8_t speed[7] = {0xA5, 0x5A ,0x05 ,0xA8, 0x32 ,0xDF,  0xAA};
	   write(fd_, stop, 6);
	   uint8_t clo[6] = {0xA5, 0x5A, 0x04, 0xE4, 0xE8, 0xAA};
	   usleep(1000 * 1000);
	   write(fd_, mode, 6);
	   // write(fd_, speed, 7);
	   write(fd_, clo, 6);
  ...
  while (n.ok())
  {
	  ...
	  if(model == "100S" && data_raw[kk] == 0xA5 && data_raw[kk + 1] == 0x5A)
          ...
          if(model == "100D2" && data_raw[kk] == 0xA5 && data_raw[kk + 1] == 0x5A)
          {
              ...
              msg.header.stamp = ros::Time::now();
              msg.header.frame_id = frame_id;
              msg.angular_velocity.x = d2f_gyro(data + 15) / 180 * 3.1415926;
              msg.angular_velocity.y = d2f_gyro(data + 17) / 180 * 3.1415926;
              msg.angular_velocity.z = d2f_gyro(data + 19) / 180 * 3.1415926;
